  ...
  return (
    <div className="space-y-6">
      ...
      {fetching ? (
        ...
        (() => {
          const currentWeekly = forecast.currentWeeklyDemand ?? 0;
          const predicted = forecast.predictedDemand ?? 0;
          const changePercent = forecast.expectedChangePercent ?? 0;
          const shortage = forecast.potentialShortage ?? 0;
          const season = forecast.season || 'Rabi Harvest';
              ...
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                <div className="card p-5">
                  ...
                  <p className="text-2xl font-bold text-gray-900 mt-1">
                    {currentWeekly.toLocaleString('en-IN')} kg
                  </p>
                ...
                <div className="card p-5">
                  ...
                  <p className="text-2xl font-bold text-primary-700 mt-1">
                    {predicted.toLocaleString('en-IN')} kg
                  </p>
                ...
                <div className={`card p-5 ${shortage > 0 ? 'bg-amber-50/50 border-amber-200' : ''}`}>
                  <span className="text-xs text-gray-500 block">Potential Supply Gap</span>
                  <div className="flex items-center gap-1.5 mt-1">
                    {shortage > 0 && <AlertTriangle className="w-5 h-5 text-amber-600" />}
                    <p className="text-2xl font-bold text-amber-700">
                      {shortage > 0
                        ? `${(shortage / 1000).toFixed(1)}T`
                        : 'Balanced'}
                    </p>
                  </div>
                  <span className="text-[11px] text-amber-700">
                    {shortage > 0 ? 'Estimated regional shortfall' : 'Adequate supply buffer'}
                  </span>
                </div>
              </div>
              ...
              <div className="grid lg:grid-cols-3 gap-6">
                  ...
                  <div className="space-y-4 pt-2">
                    <div>
                      <div className="flex justify-between text-xs font-semibold mb-1">
                        <span className="text-gray-600">Current Weekly Demand: {currentWeekly.toLocaleString('en-IN')} kg</span>
                    ...
                    <div>
                      <div className="flex justify-between text-xs font-semibold mb-1">
                        <span className="text-primary-700">
                          Forecasted Demand: {predicted.toLocaleString('en-IN')} kg
                        </span>
                        <span className="text-primary-700 font-bold">
                          {Math.round((predicted / (currentWeekly || 1)) * 100)}%
                        </span>
                      </div>
                      <div className="w-full bg-gray-100 rounded-full h-4 overflow-hidden">
                      ...
                      <div>
                        <div className="flex justify-between text-xs font-semibold mb-1">
                          <span className="text-amber-700">
                            Estimated Regional Shortage: {shortage.toLocaleString('en-IN')} kg
                          </span>
                ...
                <div className="card p-6 space-y-4">
                  ...
                  <div className="p-4 bg-gray-50 rounded-xl space-y-2">
                    ...
                    <p className="text-xl font-bold text-navy-900">{season}</p>
